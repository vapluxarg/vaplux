...
export default function NewProduct() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error(err)
      alert('Error guardando el producto. Verificá que la Tienda y Slug no estén duplicados. Y asegurate de haber creado el bucket público "products" manual en Supabase. Error: ' + err.message)
      setLoading(false)
    }
  }
  ...
  return (
    <div className="max-w-6xl mx-auto pb-12">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <Link href="/admin/products" className="text-sm font-bold text-slate-400 hover:text-blue-600 mb-2 inline-flex items-center gap-1">
            ← Volver al Inventario
          </Link>
          <h1 className="text-3xl font-extrabold text-slate-800 tracking-tight">Crear Producto Nuevo</h1>
        </div>
        <button 
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="md:col-span-1">
              ...
              <select value={categoryId} onChange={e => {setCategoryId(e.target.value); setSubcategoryId('');}} className="w-full bg-white border border-slate-300 rounded-sm px-3 py-2 outline-none focus:ring-1 focus:ring-blue-500 text-sm">
                ...
                {categories.filter(c => c.store === store).map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
              </select>
            </div>
          ...
          <div className="border border-slate-200 rounded-sm p-3">
            ...
            {video ? (
              <div className="flex items-center gap-3 bg-slate-50 border border-slate-200 p-2 rounded-sm">
                ...
                <button type="button" onClick={() => setVideo(null)} className="text-red-600 hover:text-red-700 text-xs font-bold hover:underline">Eliminar</button>
              </div>
            ) : (
