  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      for (const img of images) {
        imageUrls.push(await uploadFile(img.file, 'image'))
      }

      let videoUrl = null
      if (video) videoUrl = await uploadFile(video.file, 'video')
  ...
  return (
    <div className="max-w-6xl mx-auto pb-12">
      <div className="flex items-center justify-between mb-6">
        <div>
          <Link href="/admin/products" className="text-xs font-bold text-slate-400 hover:text-blue-600 mb-1 inline-flex items-center gap-1">
            ← Volver al Inventario
          </Link>
          <h1 className="text-xl font-bold text-slate-800 tracking-tight">Crear Producto Nuevo</h1>
        </div>
        <button
          onClick={handleSubmit}
          disabled={loading}
          className="bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white px-6 py-2 rounded-sm font-semibold shadow-sm transition-colors flex items-center gap-2 text-sm"
        >
          ...
          <section className="bg-white p-5 rounded-sm shadow-sm border border-slate-200 border-t-2 border-t-blue-500">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input value={title} onChange={handleTitleChange} className="w-full bg-white border border-slate-300 rounded-sm px-3 py-2 outline-none focus:ring-1 focus:ring-blue-500 text-sm" placeholder="Ej: iPhone 16 Pro Max" required />
              </div>

              <div>
                <label className="text-xs font-bold text-slate-700 block mb-1">Slug URL <span className="text-red-500">*</span></label>
                <input value={slug} onChange={e => setSlug(e.target.value)} className="w-full bg-slate-50 border border-slate-300 rounded-sm px-3 py-2 outline-none focus:ring-1 focus:ring-blue-500 font-mono text-xs text-slate-600" placeholder="iphone-16-pro-max" required />
              </div>
            ...
            <div className="space-y-3">
              ...
              {isImported && (
                <div className="bg-orange-50 border border-orange-200 rounded-sm p-3 text-[11px] text-orange-700 flex items-start gap-2">
                  ...
                  <span>Producto importado: sin campo de stock. Se mostrará como "A pedido".</span>
                </div>
              )}
              <div>
            ...
            <div className="border border-slate-200 rounded-sm p-3">
              ...
              {video ? (
                <div className="flex items-center gap-3 bg-slate-50 border border-slate-200 p-2 rounded-sm">
                  ...
                  <span className="flex-1 text-xs font-bold text-slate-600 truncate">{video.file.name}</span>
                  <button type="button" onClick={() => setVideo(null)} className="text-red-600 text-xs font-bold hover:underline">Eliminar</button>
                </div>
              ) : (
...
NewProduct.getLayout = page => <AdminLayout>{page}</AdminLayout>
