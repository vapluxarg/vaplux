...
export async function getServerSideProps(context) {
  ...
  if (prodError) {
    ...
    return { props: { category: categoryData, dbProducts: [], dbSubcategories: [] } };
  }
...
export default function CatalogCategory({ category, dbProducts = [], dbSubcategories = [] }) {
  const [filters, setFilters] = useState({ query: '', priceMin: '', priceMax: '', sort: 'relevance', subcategory: '', brand: '', promoOnly: false })
  const [showFilters, setShowFilters] = useState(false)
  const { getProductPrice, currency, dolarBlue } = useCurrency()

  useEffect(() => {
    if (category?.id) {
      trackCategoryEvent('view', category.id)
    }
  }, [category?.id])
  ...
  const filtered = useMemo(() => {
    ...
    let list = dbProducts.filter(p => {
      ...
      const matchSub = filters.subcategory === '' || p.subcategory === filters.subcategory
      ...
      return matchQuery && matchSub && matchBrand && matchPrice && matchPromo
    })

    if (filters.tag === 'PROMO') {
      list = list.filter(p => p.has_promo)
    } else if (filters.tag) {
      list = list.filter(p => p.name.toUpperCase().includes(filters.tag))
    }
      ...
      <main className="max-w-[1400px] mx-auto px-4 py-8 flex flex-col md:flex-row gap-6 lg:gap-8 relative items-start">
        ...
        <div className="md:hidden sticky top-[72px] z-[50] w-full mb-4 px-2">
          ...
          >
            ...
            <span>Filtros y subcategorías</span>
            { (filters.subcategory || filters.brand || filters.promoOnly || filters.priceMin || filters.priceMax) && (
        ...
        `}>
          ...
          <div className={`p-6 md:p-0 md:pr-4 flex-1 overflow-y-auto ${showFilters ? 'bg-white' : ''}`}>
            <div className="mb-4">
              <a href="/catalog" className="text-sm text-blue-600 font-semibold hover:underline flex items-center gap-1">
                <span className="text-lg">‹</span> Volver al catálogo
              </a>
            </div>
            <h1 className="text-2xl font-bold mb-6 text-gray-900 hidden md:block">{category.name}</h1>
            ...
            {dbSubcategories.length > 0 && (
              <div className="mb-4">
                <h3 className="font-bold text-[11px] uppercase tracking-widest mb-3 text-gray-400">Subcategorías</h3>
                <ul className="space-y-1 text-sm text-gray-600 max-h-48 overflow-y-auto custom-scrollbar pr-2">
                  <li>
                    <button 
                      ...
                      className={`hover:text-blue-600 transition-colors w-full text-left py-1 ${filters.subcategory === '' ? 'font-bold text-blue-600' : ''}`}
                    >
                  ...
                  {dbSubcategories.map(s => (
                    <li key={s.id}>
                      <button 
                        onClick={() => setFilters({...filters, subcategory: s.name})}
                        className={`hover:text-blue-600 transition-colors w-full text-left py-1 ${filters.subcategory === s.name ? 'font-bold text-blue-600' : ''}`}
                      >
                        {s.name}
                      </button>
                    </li>
        ...
        <div className="flex-1 min-w-0">
          ...
          {filtered.length === 0 ? (
            <div className="bg-white rounded-lg p-8 text-center shadow-sm">
              <p className="text-gray-500 text-lg">No hay productos que coincidan con los filtros en {category.name}.</p>
            </div>
          ) : (
