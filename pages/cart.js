...
export default function Cart(){
  const { items, setQty, remove, totalItems, coupon, recordCouponUse } = useCart()
        ...
        {!items.length ? (
          ...
          <div className="grid lg:grid-cols-12 gap-12 items-start">
            ...
            <div className="lg:col-span-8 space-y-4">
              {items.map(p => (
                <div key={p._cartKey || p.id} className="group bg-white rounded-[2.5rem] p-6 md:p-8 border border-slate-100 shadow-[0_8px_30px_rgba(0,0,0,0.02)] flex items-center gap-6 transition-all hover:shadow-xl">
                  {/* Image */}
                  <div className="w-24 h-24 md:w-32 md:h-32 bg-slate-50 rounded-[1.5rem] flex-shrink-0 relative overflow-hidden">
                     {(p.image_urls?.[0] || p.image) ? (
                        <img src={p.image_urls?.[0] || p.image} alt={p.title || p.name} className="w-full h-full object-contain p-2" />
                     ) : (
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-2 mb-4">
                      <div>
                        <h3 className="font-black text-slate-900 text-lg md:text-xl tracking-tight line-clamp-1">{p.title || p.name}</h3>
                        {p._variantLabel && (
                          <p className="text-slate-400 text-xs font-semibold">{p._variantLabel}</p>
                        )}
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4 bg-slate-50 rounded-2xl p-1 px-2 border border-slate-100">
                        <button 
                          onClick={() => setQty(p._cartKey || p.id, Math.max(1, p.qty - 1))}
                        ...
                        <button 
                          onClick={() => setQty(p._cartKey || p.id, p.qty + 1)}
                          className="w-10 h-10 flex items-center justify-center text-slate-400 hover:text-blue-600 transition-colors"
                        >
                          <Plus size={18} />
                        </button>
                      ...
                      <button 
                        onClick={() => remove(p._cartKey || p.id)}
            ...
            <div className="lg:col-span-4 sticky top-24">
              <div className="bg-slate-900 rounded-[3rem] p-10 text-white relative overflow-hidden shadow-2xl shadow-blue-900/20">
                ...
                {/* Coupon Input */}
                <div className="relative z-10 mb-6 bg-white/5 rounded-2xl p-4 border border-white/10">
                  <CouponInput compact />
                </div>

                <div className="space-y-3 mb-8 relative z-10">
                  {/* Subtotal */}
                  <div className="flex justify-between text-white/50 font-medium text-sm">
                  ...
                  <div className="pt-4 border-t border-white/10 flex justify-between items-end">
                    ...
                    <div className="text-right">
                       <span className="block text-3xl font-black tracking-tighter text-blue-400">{formatPrice(totalWithDiscount)}</span>
                    </div>
                  </div>
                </div>
